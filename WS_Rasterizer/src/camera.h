...
namespace cameras {
	...
	public:
		...
		: m_origin(origin), m_focalLength(focalLength), m_filmWidth(filmWidth), m_filmHeight(filmHeight) , m_transformation(transformation),
		...
		*/
		geometry::Triangle_3D<float> projectToCamera(geometry::Triangle_3D<float> tri){
			...
			for (geometry::Vertex_3D<float> vert : tri.getVertices()) {
				...
				geometry::Vector_2D<int> sv(v.x / v.z, (v.y / -v.z));
				...
				geometry::Vector_2D<int> ndc(	(sv.x - this->m_left) / (this->m_right - this->m_left), // may coz float errors (deviding small close numbers)
